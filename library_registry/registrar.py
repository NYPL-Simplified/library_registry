...
class LibraryRegistrar:
    ...
    def register(self, library, library_stage):
        ...
        if auth_document.id != auth_response.url:
            msg = "The OPDS authentication document's id (%(id)s) doesn't match its url (%(url)s)."
            failure_detail = lgt(msg, id=auth_document.id, url=auth_response.url)
        ...
        if opds_response.status_code == 401:
            ...
            if content_type != AuthenticationDocument.MEDIA_TYPE:
                msg = "401 response at %(url)s did not yield an Authentication For OPDS document"
                failure_detail = lgt(msg, url=opds_url)
            elif not self.opds_response_links_to_auth_document(opds_response, auth_url):
                msg = "Authentication For OPDS document guarding %(opds_url)s does not match the one at %(auth_url)s"
                failure_detail = lgt(msg, opds_url=opds_url, auth_url=auth_url)
            ...
            msg = "OPDS root document at %(opds_url)s does not link back to authentication document %(auth_url)s"
            failure_detail = lgt(msg, opds_url=opds_url, auth_url=auth_url)
    ...
    def _make_request(self, registration_url, url, on_404, on_timeout, on_exception, allow_401=False):
        ...
        try:
            ...
            if not allow_401 and response.status_code == 401:
                self.log.error("Registration of %s failed: %s is behind authentication gateway", registration_url, url)
    ...
    def _required_email_address(cls, uri, problem_title):
        ...
        if not uri:
            ...
            problem = on_error.detailed(
                lgt("URI must start with 'mailto:' (got: %s)") % uri
            )
        if problem:
            problem.title = problem_title
            return problem
