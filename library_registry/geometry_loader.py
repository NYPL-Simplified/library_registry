...
from library_registry.model import (get_one_or_create, Place, PlaceAlias)
...
class GeometryLoader:
    ...
    def load(self, metadata, geometry):
        ...
        # This gives us a Geometry object. Set its SRID so the db knows it's using real-world lat/lon.
        geometry = GeometryUtility.from_geojson(geometry)
        (place, place_is_new) = get_one_or_create(
            self._db, Place, external_id=external_id, type=type,
            parent=parent, create_method_kwargs=dict(geometry=geometry)
        )
        ...
        # We only ever add aliases. If the database contains an alias for this place that doesn't show
        # up in the metadata, it may have been created manually.
        for alias in aliases:
            name = alias['name']
            language = alias['language']
            (alias, _) = get_one_or_create(self._db, PlaceAlias, place=place, name=name, language=language)

        self.places_by_external_id[external_id] = place

        return place, place_is_new
