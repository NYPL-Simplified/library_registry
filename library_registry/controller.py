...
from library_registry.config import (CannotLoadConfiguration, CannotSendEmail, Configuration)
...
class LibraryRegistry:
    ...
    def setup_controllers(self, emailer_class=Emailer):
        ...
        self.view_controller        = ViewController(self)                              # noqa: E221
        self.registry_controller    = LibraryRegistryController(self, emailer_class)    # noqa: E221
        self.validation_controller  = ValidationController(self)                        # noqa: E221
        self.coverage_controller    = CoverageController(self)                          # noqa: E221
        self.static_files           = StaticFileController(self)                        # noqa: E221
        self.heartbeat              = HeartbeatController()                             # noqa: E221
...
class LibraryRegistryAnnotator(Annotator):
    ...
    def annotate_catalog(self, catalog, live=True):
        ...
        catalog.add_link_to_catalog(catalog.catalog, href=register_url, rel="register",
        ...
        (vendor_id, _, _) = Configuration.vendor_id(self.app._db)
...
class StaticFileController(BaseController):
    def static_file(self, directory, filename):
        return send_from_directory(directory, filename, cache_timeout=None)


class ViewController(BaseController):
    def __call__(self):
        username = session.get('username', '')
        return Response(render_template_string(admin_template, username=username))
...
class LibraryRegistryController(BaseController):
    ...
    def nearby(self, location, live=True):
        ...
        catalog = OPDSCatalog(self._db, str(lgt("Libraries near you")), this_url,
                              qu, annotator=self.annotator, live=live)
    ...
    def search(self, location, live=True):
        ...
        if query:       # Run the query and send the results.
            results = Library.search(self._db, location, query, production=live)
            this_url = self.app.url_for(search_controller, q=query)
            catalog = OPDSCatalog(self._db, str(lgt('Search results for "%s"')) % query,
                                  this_url, results, annotator=self.annotator, live=live)
            return catalog_response(catalog)
        else:           # Send the search form.
            ...
            headers = {
                'Content-Type': OPENSEARCH_MEDIA_TYPE,
                'Cache-Control': "public, no-transform, max-age: %d" % (3600 * 24 * 30),
            }
    ...
    def libraries(self, live=True):
        """
        Return a specific set of information about all libraries in production; this generates the
        library list in the admin interface. If :param live is set to False, libraries in testing
        will also be shown.
        """
        result = []
        ...
        # Run a single database query to get patron counts for all relevant libraries, rather than
        # calculating this one library at a time.
    ...
    def libraries_opds(self, live=True, location=None):
        ...
        :param location: If this is set, then libraries near this point will be promoted out of the alphabetical list.
        """
        alphabetical = self._db.query(Library).order_by(Library.name)
        ...
        # Pick up each library's hyperlinks and validation information; this will save
        # database queries when building the feed.
        alphabetical = alphabetical.options(
        ...
        if not location:    # No location data is available. Use the alphabetical list as the list of libraries.
            a = time.time()
            libraries = alphabetical.all()
            b = time.time()
            self.log.info("Built alphabetical list of all libraries in %.2fsec" % (b-a))
        else:
            # Location data is available. Get the list of nearby libraries, then get the rest of the
            # list in alphabetical order.

            # We can't easily do the joindeload() thing for this query, because it doesn't simply return
            # Library objects, but it won't return more than five results.
            a = time.time()
            nearby_libraries = Library.nearby(self._db, location, production=live).limit(5).all()
            b = time.time()
            self.log.info("Fetched libraries near %s in %.2fsec" % (location, b-a))

            # Exclude nearby libraries from the alphabetical query to get a list of faraway libraries.
            faraway_libraries = alphabetical.filter(~Library.id.in_([x.id for x, distance in nearby_libraries]))
            c = time.time()
    ...
    def library_details(self, uuid, library=None, patron_count=None):
        ...
        # It's presumed that associated Hyperlinks and ConfigurationSettings were loaded using joinedload(),
        # as a performance optimization. To avoid further database access, we'll iterate over the preloaded
        # objects and put the information into Python data structures.
        ...
        (
            contact_email_hyperlink,
            help_email_hyperlink,
            copyright_email_hyperlink
        ...
        for s in library.settings:
            if s.key not in setting_types or s.external_integration is not None:
                continue
            # We use _value to access the database value directly, instead of the 'value' hybrid property,
            # which creates the possibility that we'll have to go to the database to try to find a default
            # we know isn't there.
        ...
        stages = {"library_stage": library._library_stage, "registry_stage": library.registry_stage}
    ...
    def validate_email(self):
        ...
        (validation, _) = get_one_or_create(self._db, Validation, resource=hyperlink.resource)
        validation.restart()
    ...
    def edit_registration(self):
        """
        Edit a specific library's registry_stage and library_stage based on information which an
        admin has submitted in the interface.
        """
        uuid = request.form.get("uuid")
        library = self.library_for_request(uuid)
    ...
    def library(self):
        ...
        catalog = OPDSCatalog(self._db, library.name, this_url, [library],
    ...
    def catalog_response(self, document, status=200):
        ...
        return Response(document, status, headers={"Content-Type": OPDS_CATALOG_REGISTRATION_MEDIA_TYPE})
    ...
    def register(self, do_get=HTTP.debuggable_get):
        ...
        # If 'stage' is not provided, it means the client doesn't make the testing/production distinction.
        # We have to assume they want production -- otherwise they wouldn't bother registering.
        ...
        # NOTE: This is commented out until we can say that registration requires providing a contact
        # email and expect every new library to be on a circulation manager that can meet this requirement.
        #
        ...
        # Registration is a complex multi-step process. Start a subtransaction so we can back out of the
        # whole thing if any part of it fails.
        ...
        if shared_secret:
            # Look up a library by the provided shared secret. This will let us handle the case where the
            # library has changed URLs (auth_url does not match library.authentication_url) but the shared
            # secret is the same.
            ...
            if library.authentication_url != auth_url:
                # The library's authentication URL has changed, e.g. moved from HTTP to HTTPS. The registration
                # includes a valid shared secret, so it's okay to modify the corresponding database field.
                #
                # We want to do this before the registration, so that we request the new URL instead of the old one.
        ...
        # At this point registration (or re-registration) has succeeded, so we won't be rolling back
        # the subtransaction that created the Library.
        __transaction.commit()
        (auth_document, hyperlinks_to_create) = result

        # Now that we've completed the registration process, we know the opds_url -- it's the 'start' link
        # found in the auth_document.
        #
        # Registration will fail if this link is missing or the URL doesn't work, so we can assume this is valid.
        ...
        # The registration process may have queued up a number of Hyperlinks that needed to be created (taken
        # from the library's authentication document), but we also need to create a hyperlink for the integration
        # contact provided with the registration request itself.
        ...
        if elevated_permissions:
            ...
            if library.opds_url != opds_url:
                # The library's OPDS URL has changed, e.g. moved from HTTP to HTTPS. Since we have elevated
                # permissions, it's okay to modify the corresponding database field.
                library.opds_url = opds_url
        ...
        for rel, candidates in hyperlinks_to_create:
            ...
            if is_modified:
                # We need to send an email to this email address about what just happened. This is either
                # so the receipient can confirm that the address works, or to inform them a new library is
                # using their address.
                try:
                    ...
                    return UNABLE_TO_NOTIFY.detailed(lgt("The Registry was unable to send a notification email."))
        ...
        # Annotate the catalog with some information specific to the transaction that's happening right now.
    ...
    def registration_document(self):
        """
        Serve a document that describes the registration process, notably the terms of service for that process.

        The terms of service are hosted elsewhere; we only know the URL of the page they're stored.
        """
        document = {}
        ...
        # And/or the terms of service may be described in human-readable HTML, which we'll present as a data: link.
...
class ValidationController(BaseController):
    ...
    The confirmation codes were sent out in emails to the addresses that need to be validated,
    or otherwise communicated to someone who needs to click on the link to this controller.
    """
    ##### Class Constants ####################################################  # noqa: E266
    ...
    def html_response(self, status_code, message):
        ...
        This controller is used by human beings, so HTML is better than Problem Detail Documents.
        """
        return Response(self.MESSAGE_TEMPLATE % {"message": message},
                        status_code, headers={"Content-Type": "text/html"})
    ...
    def confirm(self, resource_id, secret):
        ...
        if not validation:
            # The secret is invalid. This might be because the secret is wrong, or because the
            # Resource has already been validated.
            #
            # Let's eliminate the 'Resource has already been validated' possibility and take care
            # of the other case next.
        ...
        if (not validation or not validation.resource or validation.resource.id != resource_id):
            ...
            error = lgt("Confirmation code %r not found") % secret
            return self.html_response(404, error)

        # At this point we know that the resource has not been confirmed, and that the secret matches
        # the resource. The only other problem might be that the validation has expired.
        if not validation.active:
            error = lgt("Confirmation code %r has expired. Re-register to get another code.") % secret
            return self.html_response(400, error)

        validation.mark_as_successful()
        resource = validation.resource
        message = lgt("You successfully confirmed %s.") % resource.href
...
class CoverageController(BaseController):
    """Converts coverage area descriptions to GeoJSON documents so they can be visualized."""
    def geojson_response(self, document):
        if isinstance(document, dict):
            document = json.dumps(document)

        return Response(document, 200, headers={"Content-Type": "application/geo+json"})
    ...
    def lookup(self):
        ...
        (places, unknown, ambiguous) = AuthenticationDocument.parse_coverage(self._db, coverage)
        document = Place.to_geojson(self._db, *places)

        # Extend the GeoJSON with extra information about parts of the coverage document we found
        # ambiguous or couldn't associate with a Place.
        if unknown:
