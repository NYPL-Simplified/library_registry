...
class DatabaseTest:
    ...
    def teardown(self):
        secret_keys = self._db.query(ConfigurationSetting).filter(ConfigurationSetting.key==Configuration.SECRET_KEY)   # noqa: E225,E501
    ...
    # DONE
    def _library(self, name=None, short_name=None, eligibility_areas=[], focus_areas=[],
                 audiences=None, library_stage=Library.PRODUCTION_STAGE,
    ...
    # DONE
    def _external_integration(self, protocol, goal=None, settings=None, libraries=None, **kwargs):
        integration = None
        if not libraries:
    ...
    def connecticut_state_library(self):
        return self._library("Connecticut State Library", "CT", [self.connecticut_state], has_email=True)
    ...
    new_york_state_geojson = ''

    connecticut_state_geojson = ''

    new_york_city_geojson = ''

    zip_10018_geojson = ''

    zip_12601_geojson = ''

    crude_kings_county_geojson = ''

    connecticut_geojson = ''

    kansas_state_geojson = ''

    manhattan_ks_geojson = ''

    boston_geojson = ''

    zip_11212_geojson = ''

    crude_albany_geojson = ''

    new_mexico_state_geojson = ''

    crude_new_york_county_geojson = ''
