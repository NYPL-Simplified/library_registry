...
class TestGeometryLoader:
    def test_load(self, db_session, geoloader):
        ...
        texas_zip, is_new = geoloader.load(metadata, geography)
        ...
        new_york, is_new = geoloader.load(metadata, geography)
    ...
    def test_load_ndjson(self, db_session, geoloader):
        ...
        test_ndjson_lines = [
            ('{"parent_id": null, "name": "United States", "aliases": [{"name" : "The Good Old U. S. of A.", '
             '"language": "eng"}], "type": "nation", "abbreviated_name": "US", "id": "US"}'),
            '{"type": "Point", "coordinates": [-159.459551, 54.948652]}',
            ('{"parent_id": "US", "name": "Alabama", "aliases": [], "type": "state", '
             '"abbreviated_name": "AL", "id": "01"}'),
            '{"type": "Point", "coordinates": [-88.053375, 30.506987]}',
            ('{"parent_id": "01", "name": "Montgomery", "aliases": [], "type": "city", '
             '"abbreviated_name": null, "id": "0151000"}'),
            '{"type": "Point", "coordinates": [-86.034128, 32.302979]}',
        ]
        input = StringIO("\n".join(test_ndjson_lines))
        [(us, _), (alabama, _), (montgomery, _)] = list(geoloader.load_ndjson(input))
        ...
        # The place that existed before we ran the loader is still the same database object, but it has had
