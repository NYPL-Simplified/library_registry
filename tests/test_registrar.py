...
from library_registry.problem_details import (NO_AUTH_URL, INVALID_CONTACT_URI)
...
class TestRegistrar:
    # TODO: The core method, register(), is tested indirectly in test_controller.py, because
    # the LibraryRegistrar code was originally part of LibraryRegistryController. This could be
    # refactored.
    ...
    def test_opds_response_links(self):
        ...
        This method is used to find the link back from the OPDS document to the Authentication For OPDS document.

        It checks the Link header and the body of an OPDS 1 or OPDS 2 document.

        This test also tests the related opds_response_links_to_auth_document, which checks whether a
        ...
        assert set(LibraryRegistrar.opds_response_links(response, rel)) == set([auth_url, "http://another-auth-document"])  # noqa: E501
        assert LibraryRegistrar.opds_response_links_to_auth_document(response, auth_url) is True
    ...
    def test__required_email_address(self):
        ...
        problem = LibraryRegistrar._required_email_address(None, 'a title')
        ...
        problem = LibraryRegistrar._required_email_address("http://not-an-email/", "a title")
        assert problem.uri == uri
        assert problem.detail == "URI must start with 'mailto:' (got: http://not-an-email/)"

        mailto = "mailto:me@library.org"
        success = LibraryRegistrar._required_email_address(mailto, "a title")
