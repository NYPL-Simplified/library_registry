...
class TestOPDSCatalog:
    ...
    def test_library_catalogs(self, db_session, create_test_library):
        ...
        db_session.delete(l1)
        db_session.delete(l2)
        db_session.commit()

    def test_large_feeds_treated_differently(self, db_session):
        """
        The libraries in large feeds are converted to JSON in ways that omit large chunks of data
        such as inline logos.
    ...
    def test_feed_is_large(self, db_session, create_test_library):
        ...
        l1 = create_test_library(db_session)
        l2 = create_test_library(db_session)
    ...
    def test_library_catalog(self, db_session, create_test_library, new_york_city):
        ...
        # The distance between the current location and the edge of the library's service area is published
        # as 'schema:distance' and also (for backwards compatibility) as 'distance'
        for key in ('schema:distance', 'distance'):
            assert metadata[key] == '14 km.'

        # The library's updated timestamp is published as 'modified' and (for backwards compatibility) as 'updated'.
        ...
        # If library_catalog is called with include_logo=False, the (potentially large) inline logo is
        # omitted, even though it was included before.
        ...
        catalog = Mock.library_catalog(library, url_for=self.mock_url_for, include_service_area=False)
        ...
        catalog = Mock.library_catalog(library, url_for=self.mock_url_for, include_service_area=True)
