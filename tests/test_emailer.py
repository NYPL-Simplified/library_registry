...
class TestEmailer(DatabaseTest):
    ...
    def test__sitewide_integration(self):
        ...
        with pytest.raises(CannotLoadConfiguration) as exc:
            m(self._db)
        assert 'No email integration is configured' in exc.value
        ...
        with pytest.raises(CannotLoadConfiguration) as exc:
            m(self._db)
        assert 'Multiple email integrations are configured' in exc.value
    ...
    def test_constructor(self):
        ...
        with pytest.raises(CannotLoadConfiguration) as exc:
            m(**args)
        assert "No SMTP username specified" in exc.value

        args['smtp_username'] = 'user'

        with pytest.raises(CannotLoadConfiguration) as exc:
            m(**args)
        assert "No SMTP password specified" in exc.value

        args['smtp_password'] = 'password'

        with pytest.raises(CannotLoadConfiguration) as exc:
            m(**args)
        assert "No SMTP host specified" in exc.value

        args['smtp_host'] = 'host'

        with pytest.raises(CannotLoadConfiguration) as exc:
            m(**args)
        assert "No SMTP port specified" in exc.value

        args['smtp_port'] = 'port'

        with pytest.raises(CannotLoadConfiguration) as exc:
            m(**args)
        assert "No From: name specified" in exc.value

        args['from_name'] = 'Email Sender'

        with pytest.raises(CannotLoadConfiguration) as exc:
            m(**args)
        assert "No From: address specified" in exc.value
        ...
        with pytest.raises(CannotLoadConfiguration) as exc:
            m(**args)
        assert "Template '%\(nope\)s'/'email body' contains unrecognized key" in exc.value
