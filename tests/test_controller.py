...
class TestBaseController(ControllerTest):
    ...
    def test_library_for_request(self):
        ...
        with self.app.test_request_context("/"):
            ...
            assert request.library == library

            request.library = None
...
class TestLibraryRegistryController(ControllerTest):
    ...
    def _is_library(self, expected, actual, has_email=True):
        # Helper method to check that a library found by a controller is
        # equivalent to a particular library in the database
        flattened = {}
        # Getting rid of the "uuid" key before populating flattened, because
        # its value is just a string, not a subdictionary.
    ...
    def test_libraries_opds_qa(self):
        library = self._library(                    # noqa: F841
    ...
    def test_edit_registration_with_override(self):
        ...
        with self.app.test_request_context("/", method="POST"):
            ...
            edited_nypl = get_one(self._db, Library, internal_urn=nypl.internal_urn)    # noqa: F841
    ...
    @pytest.mark.skip(reason="outdated, replace with new search testing")
    ...
    def test_register_success(self):
        ...
        with self.app.test_request_context("/", method="POST"):
            ...
            # document.
            #
            assert self.http_client.requests == ["http://circmanager.org/authentication.opds",
        ...
        with self.app.test_request_context("/", headers={"Authorization": "Bearer %s" % old_secret}, method="POST"):
            request.form = form_args_with_reset
            key = RSA.generate(1024)
            auth_document = self._auth_document(key)
            self.http_client.queue_response(
                200, content=json.dumps(auth_document), url=auth_document['id']
