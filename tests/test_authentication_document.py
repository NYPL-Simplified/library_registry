...
class TestParseCoverage:
    ...
    def parse_places(self, db_session, coverage_object, expected_places=None,
        ...
        """
        place_objs, unknown, ambiguous = AuthDoc.parse_coverage(db_session, coverage_object, MockPlace)
    ...
    def test_ambiguous_country(self, db_session):
        # Test the unlikely scenario where an authentication document says a library covers an entire country,
        # but it's ambiguous which country is being referred to.
    ...
    def test_unknown_country(self, db_session):
        # Test an authentication document that says a library covers an entire country, but the library registry
        # doesn't know anything about that country's geography.
    ...
    def test_places_within_country(self, db_session):
        # Test an authentication document that says a library covers one or more places within a country.

        # This authentication document covers two places called "San Francisco" (one in the US and one
        # in Mexico) as well as a place called "Mexico City" in Mexico.
        #
        # Note that it's invalid to map a country name to a single place name (it's supposed to always
        # be a list), but our parser can handle it.
    ...
    def test_unscoped_place_is_in_default_nation(self, db_session):
        ...
        # With a default nation in place, a bare string like "CA" is treated the same as a correctly
        # formatted dictionary like {"US": ["CA"]}
...
class TestLinkExtractor:
    ...
    def test_empty_document(self):
        ...
        # In the absence of specific information, we assume the most common case: a public library
        # that simply hasn't specified its service area.

        # A blank auth document is created with
    ...
    def test_real_document(self):
        ...
        place = MockPlace()
        everywhere = place.everywhere(None)     # noqa: F841
...
class TestUpdateServiceAreas:
    ...
    def test_ambiguous_and_unknown_places_become_problemdetail(
        ...
        valid = [create_test_place(db_session)]
    ...
    def test_update_service_areas(self, db_session, create_test_library, create_test_place):
        ...
        doc_dict = dict(
            ...
            focus_area={country1.abbreviated_name: everywhere, country2.abbreviated_name: everywhere}
        )
        ...
        for db_item in (library, country1, country2, country3):
    ...
    def test_service_area_registered_as_focus_area_if_identical_to_focus_area(
        ...
        everywhere = AuthenticationDocument.COVERAGE_EVERYWHERE
        doc_dict = dict(service_area=everywhere, focus_area=everywhere)
        doc = AuthenticationDocument.from_dict(db_session, doc_dict)
        problem = doc.update_service_areas(library)
        db_session.commit()
        assert problem is None
...
class TestUpdateCollectionSize:
    ...
    def test_success(self, db_session, create_test_library):
        ...
        # Two CollectionSummaries have been created, for the English collection and the (empty) Japanese collection.
    ...
    def test_unknown_language_registered_as_unknown(self, db_session, create_test_library):
        ...
        # Here's a tricky case with multiple unknown languages.  They all get grouped together into
        # a single 'unknown language' collection.
