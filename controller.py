...
from flask import (Response, g, redirect, render_template_string, request,
                   session, url_for)
...
from model import (Admin, ConfigurationSetting, Hyperlink, Library, Place,
                   ...
                   get_one_or_create, production_session)
...
OPDS_CATALOG_REGISTRATION_MEDIA_TYPE = "application/opds+json;profile=https://librarysimplified.org/rel/profile/directory"  # noqa: E501
...
class LibraryRegistryAnnotator(Annotator):
    ...
    def annotate_catalog(self, catalog, live=True):
        ...
        search_controller = "search" if live else "search_qa"
...
class ViewController(BaseController):
    def __call__(self):
        ...
        response = Response(render_template_string(admin_template, username=username))
        return response
...
class LibraryRegistryController(BaseController):
    ...
    def __init__(self, app, emailer_class=Emailer):
        ...
        try:
            ...
            msg = "Cannot load email configuration. Will not be sending any emails."
            self.log.error(msg, exc_info=e)

        self.emailer = emailer

    def nearby(self, location, live=True):
        if 'location' not in g or not isinstance(g.location, Location):
            return 'foo'

        qu = Library.nearby(self._db, g.location, production=live)
        qu = qu.limit(5)
        nearby_controller = 'nearby' if live else 'nearby_qa'
    ...
    def search(self, location, live=True):
        ...
        if query:       # Run the query and send the results.
            ...
            headers = {
                ...
                'Cache-Control': f"public, no-transform, max-age: {3600 * 24 * 30}",
            }
    ...
    def _format_place_name(self, place):
        if place.parent:
            parent_name = (place.parent.abbreviated_name or place.parent.external_name)
        else:
            parent_name = "unknown"
    ...
    def edit_registration(self):
        """
        Edit a specific library's registry_stage and library_stage based on information
        which an admin has submitted in the interface.
        """
        uuid = request.form.get("uuid")
        library = self.library_for_request(uuid)
    ...
    def render(self):
        return Response(render_template_string(admin_template))
    ...
    def register(self, do_get=HTTP.debuggable_get):
        ...
        if not library:
            # This is a library at a known authentication URL or a brand new library.
        ...
        # At this point registration (or re-registration) has succeeded, so we won't be rolling
        # back the subtransaction that created the Library.
        __transaction.commit()
        auth_document, hyperlinks_to_create = result

        # Now that we've completed the registration process, we know the opds_url -- it's the
        # 'start' link found in the auth_document.
        # Registration will fail if this link is missing or the URL doesn't work, so we can
        # assume this is valid.
        opds_url = auth_document.root['href']

        if library_is_new:
            # The library was just created, so it had no opds_url. Set that now.
            library.opds_url = opds_url

        # The registration process may have queued up a number of Hyperlinks that needed to
        # be created (taken from the library's authentication document), but we also need to
        # create a hyperlink for the integration contact provided with the registration request itself.
        ...
        if elevated_permissions:
            ...
            if library.opds_url != opds_url:
                # The library's OPDS URL has changed, e.g. moved from HTTP to HTTPS. Since we
                # have elevated permissions, it's okay to modify the corresponding database field.
                library.opds_url = opds_url
        ...
        for rel, candidates in hyperlinks_to_create:
            ...
            if is_modified:
                ...
                # This is either so the receipient can confirm that the address works, or to inform
        ...
        # Annotate catalog with some info specific to the transaction happening right now.
        ...
        status_code = 201 if library_is_new else 200
...
class ValidationController(BaseController):
    ...
    def html_response(self, status_code, message):
        ...
        return Response(page, status_code, headers={"Content-Type": "text/html"})
    ...
    def confirm(self, resource_id, secret):
        ...
        if not validation:
            ...
            # Let's eliminate the 'Resource has already been validated' possibility and
            # take care of the other case next.
        ...
        if (not validation or not validation.resource or validation.resource.id != resource_id):
            # For whatever reason the resource ID and secret don't match. A generic error that
            # doesn't reveal information is appropriate in all cases.
            error = lgt("Confirmation code %r not found") % secret
            return self.html_response(404, error)

        # At this point we know the resource has not been confirmed, and that the secret matches
